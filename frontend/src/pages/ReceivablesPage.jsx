...
export default function ReceivablesPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const data = await ledgerService.getReceivables({ branchId });
      setRows(data);
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  // Customers owe us: positive = Dr (they are debited), negative = Cr
  const drCr = (n) => Number(n) >= 0 ? 'Dr' : 'Cr';
  const amtClass = (n) => Number(n) >= 0 ? 'ledger-debit' : 'ledger-credit';
    ...
    <div className="dashboard-stack">
      <div className="page-stats-strip">
        ...
        <div className="page-stat-tile page-stat-tile--danger">
          <span className="page-stat-tile__label">Total Receivable</span>
      ...
      <PageCard
        ...
        subtitle="Outstanding amounts owed by customers"
        actions={
      ...
      >
        ...
        {user?.role === 'main_admin' && (
          <div className="table-filters">
            <label className="form-field" htmlFor="receivablesBranch">
              <span>Branch *</span>
              <select
                id="receivablesBranch"
              ...
              {rows.length > 0 ? (
                <tfoot>
                  <tr className="table-total-row">
                    <td colSpan="2">
                      <strong>Total Customer Balance</strong>
                    </td>
